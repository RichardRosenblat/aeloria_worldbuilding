const rychScript = (genOptions, numberOfLastNames, specialTreatment = 'none', numberOfNamesToGenerate = 100) => {

    const specialTreatments = {
        none: [],
        'sindarin-quenya': [
            (numberOfNamesToGenerate * 2) / 10,
            (arrayOfNames) => arrayOfNames.reduce((acc, name, i) => {
                if ([1, 3, 5, 7, 9].includes(i)) {
                    const parenthesisPattern = /\([\w\/\'\’\-\s]+\)/g
                    const parenthesisPatternReplacement = /\) \+ \(/g
                    acc[acc.length - 1] += name.match(parenthesisPattern)
                        .join(' + ')
        ...
        'make-first-letters-uppercase': [
            , ,
            (arrayOfNames) => {
                return arrayOfNames.map(name => name ? name.split(' ').map(word => word[0].toUpperCase() + word.slice(1)).join(' ') : name)
            }
        ]
    }
    ...
    const generateNames = (genType) => {
        ...
        for (let i = 0; i < loops; i++) {
            nameGen(genType);
            const arrayOfNames = preTreatment(document.getElementById('result').innerHTML.split('<br>').filter(Boolean));
    ...
    genOptions.forEach((genType, i) => {
        ...
        if (numberOfLastNames >= 1) {
            ...
            if (numberOfLastNames === 1) {
                console.log('------------------Second-----------------------')
                result = generateNames(genType).map(name => {
                ...
                for (let i = 1; i <= numberOfLastNames; i++) {
                    result = generateNames(genType).map(name => name.split(' ')[i]).filter(n => !!n && n !== 'undefined')
...
rychScript([undefined, 1], 1, 'make-first-letters-uppercase')